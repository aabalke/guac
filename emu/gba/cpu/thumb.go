...
func (c *Cpu) ThumbJumpCalls(op uint16) {
	...
	if !c.Reg.CPSR.CheckCond(uint32(op>>8) & 0xF) {
		c.Seq = SEQ
		return
	}
...
func (c *Cpu) ThumbBlock(op uint16) {
	var (
		r     = &c.Reg.R
		ldmia = (op>>11)&1 != 0
		rb    = (op >> 8) & 7
