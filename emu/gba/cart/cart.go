...
func (c *Cartridge) getDeviceManufacturer() int {
    ...
    switch code {
        case 0xD4BF: return TYPE_SST
        case 0x1CC2: return TYPE_MACRONIX64
        case 0x1B32: return TYPE_PANASONIC
        case 0x3D1F: return TYPE_ATMEL
        case 0x1362: return TYPE_SANYO
        case 0x09C2: return TYPE_MACRONIX128
    }
...
func (c *Cartridge) load() {
    ...
    for i := range len(sBuf) {
        switch c.Id {
        case SRAM: c.SRAM[i] = uint8(sBuf[i])
        case EEPROM: c.Eeprom[i] = uint8(sBuf[i])
        default: c.Flash[i] = uint8(sBuf[i])
        }
    }
}
...
func (c *Cartridge) Save() {
    ...
    var bytes []uint8
    switch c.Id {
    case SRAM: bytes = c.SRAM[:]
    case EEPROM: bytes = c.Eeprom[:]
    default: bytes = c.Flash[:]
    }
...
func (c *Cartridge) Read(addr uint32) uint8 {
    ...
    default: return c.ReadFlash(addr)
    }
}

func (c *Cartridge) Write(addr uint32, v uint8) {
    switch c.Id {
    case SRAM: c.SRAM[addr] = v
    case EEPROM: panic("EEPROM WRITE USING STD IMPLIMENTATION")
    default: c.WriteFlash(addr, v)
    }
}
