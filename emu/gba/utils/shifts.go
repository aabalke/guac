...
func Shift(s ShiftArgs) (shift uint32, setCarry, carry bool) {
    switch s.SType {
    case LSL: return Lsl(s.Val, s.Is, s.IsCarry, s.Immediate)
    case LSR: return Lsr(s.Val, s.Is, s.IsCarry, s.Immediate)
    case ASR: return Asr(s.Val, s.Is, s.IsCarry, s.Immediate)
    case ROR: return Ror(s.Val, s.Is, s.IsCarry, s.Immediate, s.CurrCarry)
    default: panic("Unknown Shift Type")
    }
}
