...
func (c *Cpu) stm(block *Block) {
	...
	for reg := range 16 {
		...
		case db && decRegBitEnabled: // push
			...
			if 15-reg == PC {
				c.mem.Write32(addr, r[15-reg]+12, true)
				continue
			}

			c.mem.Write32(addr, r[15-reg], true)
		...
		case da && decRegBitEnabled:
			...
			if decReg == int(block.Rn) {
				c.mem.Write32(addr, r[decReg]+(count-1)*4, true)
			...
			if decReg == PC {
				c.mem.Write32(addr, r[decReg]+12, true)
				continue
			}

			c.mem.Write32(addr, r[decReg], true)
