...
func (v *VRAM) Init(t *rast.TextureCache, a, b *Engine) {
	...
	v.banks[0] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
	}{bank: unsafe.Pointer(&v.A), cnt: &v.CNT_A}
	v.banks[1] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
	}{bank: unsafe.Pointer(&v.B), cnt: &v.CNT_B}
	v.banks[2] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
	}{bank: unsafe.Pointer(&v.C), cnt: &v.CNT_C}
	v.banks[3] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
	}{bank: unsafe.Pointer(&v.D), cnt: &v.CNT_D}
	v.banks[4] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
	}{bank: unsafe.Pointer(&v.E), cnt: &v.CNT_E}
	v.banks[5] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
	}{bank: unsafe.Pointer(&v.F), cnt: &v.CNT_F}
	v.banks[6] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
	}{bank: unsafe.Pointer(&v.G), cnt: &v.CNT_G}
	v.banks[7] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
	}{bank: unsafe.Pointer(&v.H), cnt: &v.CNT_H}
	v.banks[8] = struct {
		bank unsafe.Pointer
		cnt  *VramCnt
...
func (vm *VRAM) WriteCNT(addr uint32, v uint8) {
	...
	case 0x245:
		...
		switch cnt.Mst {
		case 0:
			cnt.Base = uint32(0x89_0000)
		case 1:
			cnt.Base = (0x4000 * uint32(cnt.Ofs&1)) + (0x10000 * uint32(cnt.Ofs>>1))
		case 2:
			cnt.Base = 0x40_0000 + (0x4000 * uint32(cnt.Ofs&1)) + (0x10000 * uint32(cnt.Ofs>>1))
		case 3:
			vm.TextureCache.Reset()
