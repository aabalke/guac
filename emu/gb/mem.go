...
func (gb *GameBoy) SaveRam() {
    if !gb.MemoryBus.ramSaved {
        cartridge.WriteRam(gb.Cartridge.Path, gb.Cartridge.RamData)
...
func (gb *GameBoy) ReadByte(addr uint16) (uint8, error) {
    ...
    case addr == 0xFF4F: return gb.MemoryBus.VRAMBank, nil
    case addr == 0xFF70: return gb.MemoryBus.WRAMBank, nil
	}
