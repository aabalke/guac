...
func (m *Mbc3) ReadRam(c Cartridge, addr uint16) uint8 {
    ...
    switch {
    case m.RamBank >= 0x4 && m.Rtc.RtcEnabled: return m.Rtc.Temp[m.RamBank]
    case m.RamBank >= 0x4 && !m.Rtc.RtcEnabled: return m.Rtc.Rtc[m.RamBank]
    }
...
func (m *Mbc3) Handle(addr uint16, v uint8) {
    ...
    case addr < 0x4000: m.setRomBank1(v)
    case addr < 0x6000: m.setRamBank(v)
    case addr < 0x8000: m.setAdvBanking(v)
    }
}
