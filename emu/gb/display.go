	...
	for y := range height {
		for x := range width {
			...
			if !gb.Color {
				switch v {
				case 0:
					ApplyColor(gb.Palette[0], gb.Pixels, index)
				case 1:
					ApplyColor(gb.Palette[1], gb.Pixels, index)
				case 2:
					ApplyColor(gb.Palette[2], gb.Pixels, index)
				case 3:
					ApplyColor(gb.Palette[3], gb.Pixels, index)
				}
