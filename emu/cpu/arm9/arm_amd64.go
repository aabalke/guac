...
func (j *Jit) EmitBlock(op uint32) {
	...
	if psr {
		...
		j.Cmp(amd64.Imm(arm7.MODE_FIQ), amd64.Edi)
		...
		notFiq()
		j.Cmp(amd64.Imm(arm7.MODE_USR), amd64.Edi)
...
func (j *Jit) EmitCoDataReg(op uint32) {
	...
	if rd == 15 {
		panic("co data reg jit with rd == PC")
	}

	if mrc := (op>>20)&1 != 0; mrc {

		j.Mov(amd64.Imm(reg.Op), amd64.Rax)
		j.Mov(amd64.Imm(reg.Cn), amd64.Rbx)
		j.Mov(amd64.Imm(reg.Pn), amd64.Rcx)
		j.Mov(amd64.Imm(reg.Cp), amd64.Rdi)
		j.Mov(amd64.Imm(reg.Cm), amd64.Rsi)
	...
	if rd == 0 && (reg == cp15.HALT || reg == cp15.HALT2) {
		j.Movb(amd64.Imm(1), HALTED_FLAG)
		//cpu.Halted = true
		return
	}

	j.Movl(j.REG(rd), amd64.Ebx)
	j.Mov(amd64.Rbx, amd64.R8)
